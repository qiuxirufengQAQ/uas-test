    ...
    <a-form layout="horizontal" v-if="conditions && conditions.length>0">
      ...
      <a-row>
        <a-col
          ...
          :key="item.aliasKey || item.key"
        >
          <a-form-item
            :label="item.label"
            :labelCol="labelCol"
            :wrapperCol="wrapperCol"
            v-show="index<4 || advanced"
          >
            <!-- INPUT -->
            <a-input
              v-if="item.inputType === 'INPUT'"
              v-model="searchQuery[item.aliasKey || item.key]"
            />
            <!-- SELECT -->
            <a-select
              v-if="item.inputType === 'SELECT'"
              v-model="searchQuery[item.aliasKey || item.key]"
              @change="value => handleOptionChange(value,item)"
            >
            ...
            <!-- DATE PICKER -->
            <a-date-picker
              v-if="item.inputType === 'DATEPICKER'"
              v-model="searchQuery[item.aliasKey || item.key]"
            />
          </a-form-item>
        </a-col>
          ...
          <a-list-item slot-scope="item" :key="item.id" slot="renderItem">
            <a-list-item-meta>
              ...
              <template v-else>
                <span
                  slot="title"
                >{{filterColumns[0].title}}:&nbsp;{{item[filterColumns[0].dataIndex]}}</span>
              </template>
            </a-list-item-meta>
            <template v-for="(c, index) in filterColumns">
              <p
                v-if="c.dataIndex && index !== 0"
                :key="index"
              >{{c.title}}:&nbsp;{{item[c.dataIndex] | render(c, item)}}</p>
              <p
                v-else-if="index !== 0 && !c.scopedSlots"
                :key="index"
              >{{c.title}}:&nbsp;{{item | render(c, item)}}</p>
            </template>
...
export default {
  ...
  methods: {
        ...
        if (this.searchQuery[condition.aliasKey || condition.key]) {
          ...
          if (condition.inputType === "DATEPICKER") {
            if (condition.judgeType === "BETWEEN") {
              ...
              condition.judgeType === "GE" ||
              condition.judgeType === "GT"
            ) {
              value
                .hours(0)
                .minute(0)
              ...
              condition.judgeType === "LE" ||
              condition.judgeType === "LT"
            ) {
              value
                .hours(0)
                .minute(0)
    ...
    expandedRowsChange(expandedRows) {
      this.expandedRowKeys = expandedRows.filter(
        i => this.expandedRowKeys.indexOf(i) < 0
      );
      let data = undefined;
