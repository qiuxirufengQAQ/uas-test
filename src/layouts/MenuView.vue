...
export default {
  ...
  created() {
    ...
    this.$eventBus.$on("closePage", key => {
      this.remove(key);
    });
  },
  watch: {
    $route: function(newRoute) {
      ...
      if (!this.multipage) {
        ...
        const pageList = this.pageList.filter(r => r.path === newRoute.path);
        if (pageList.length > 0) {
          this.linkList = this.linkList.filter(
            key => key !== pageList[0].fullPath
          );
          this.pageList = this.pageList.filter(r => r.path !== newRoute.path);
        }
